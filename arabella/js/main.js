...
var pcName = $('.popup-add-cart .product-name');
var cpName = $('.popup-add-comparison .product-name');
...
pcBtn.click(function (event) {
	...
	var clVal = $(this).parent().parent().parent().find('.goods__card_quantity input').val();
...
let body = $('body');
let noGray = $('.wrap-gray.no-gray');

if (noGray) {
	body.addClass('_no-webp');
}
// MAIN SLIDER
...
let servicesSliderNav = $('.services__slider_nav');
if (servicesSlider.length > 0 && servicesSliderNav.length > 0) {
	servicesSlider.slick({
		slidesToShow: 1,
		slidesToScroll: 1,
		arrows: false,
		// fade: true,
		asNavFor: servicesSliderNav,
	});
	servicesSliderNav.slick({
		slidesToShow: 5,
		slidesToScroll: 1,
		asNavFor: servicesSlider,
		dots: false,
		centerMode: false,
		focusOnSelect: true,
...
// GALLERY IN TABS
let gallerySlider = $('.our-works__slider_gallery .slides');
let gallerySliderNav = $('.our-works__slider_nav .slides');
if (gallerySlider.length > 0 && gallerySliderNav.length > 0) {
	...
	gallerySliderNav.slick({
		...
		vertical: true,
		verticalSwiping: true,
		// appendArrows: $('.our-works__slider_nav_arrows'),
		nextArrow:'<button type="button" class="slick-next"><img class="svg" src="img/icons/arrow-min.svg" alt="" /></button>',
		prevArrow:'<button type="button" class="slick-prev"><img class="svg" src="img/icons/arrow-min.svg" alt="" /></button>',
		// autoplay: true,
		// autoplaySpeed: 5000,
		// speed: 1000
		responsive: [{
...
// PRODUCT GALLERY
let productGal = $('.product__gallery_slider');
let productNav = $('.product__gallery_nav');
if (productGal.length > 0 && productNav.length > 0) {
	productGal.slick({
		slidesToShow: 1,
		slidesToScroll: 1,
		arrows: false,
...
function init() {
	...
	var myMap = new ymaps.Map("map", {
		...
		controls: [],
		center: [55.741586068999894,37.949140500000006],
		// Уровень масштабирования. Допустимые значения:
		// от 0 (весь мир) до 19.
		zoom: 17
	});

	myPlacemark = new ymaps.Placemark([55.741586068999894,37.949140500000006],{
		id:'2'
	},
	{
	// Опции.
